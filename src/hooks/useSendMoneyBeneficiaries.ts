...
const useSendMoneyBeneficiaries = () => {
    ...
    if (beneficiaries) {
      return JSON.parse(beneficiaries)
        .filter((beneficiary: TBeneficiary) => {
          if (type && beneficiary.type !== type) return false;
  ...
  const handleSetBeneficiary = (beneficiary: Omit<TBeneficiary, "ownedBy">) => {
    const beneficiaries = handleGetAllBeneficiaries();
